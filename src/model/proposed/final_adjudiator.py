...
BASE = Path("/home/hslee/multiagent/results/proposed/v4/mlm")
...
for stage1_file in BASE.rglob("stage1_results.jsonl"):
    file_sets.append({
        ...
        "output_base": str(stage1_file.parent / "stage2_results_final")
    })
...
def main():
    ...
    for in_path, out_path in pairs:
        ...
        # 프롬프트 준비: noisy_question을 question 자리에 넣음 (없으면 원본 question fallback)
        prompts = [
            safe_format_prompt(
                prompt_tmpl,
                question=r.get("noisy_question", ""),
