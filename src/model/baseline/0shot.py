...
# 프로젝트 루트 찾기
PROJECT_ROOT = Path(__file__).resolve().parents[3]  # run_single.py → baseline → model → src → project_root

CONFIG = {
    "raw_data": str(PROJECT_ROOT / "data/raw/medqa/medqa_all_clean.jsonl"),
    "processed_root": str(PROJECT_ROOT / "data/processed/medqa"),
    "results_root": str(PROJECT_ROOT / "results/baseline/qwen_32"),
    "prompt_file": str(PROJECT_ROOT / "prompts/v1/baseline.txt"),
...
def parse_experiment_id(input_path: str) -> Tuple[str, str, str]:
    """
    /processed/{noise}/{level}/{seed}.jsonl 형태에서
    noise, level, seed 추출
    clean은 (clean, clean, seed_0)로 통일
    """
    p = Path(input_path).as_posix()
    if "/processed/" in p:
        tail = p.split("/processed/")[1]
        parts = tail.split("/")
